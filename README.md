...
**Repair and splice.** Every lane carries a repair rack and a splice bench, both typed-array work on the samples Web Audio already decoded in the window (`src/lib/repair.ts`, `src/lib/splice.ts`). Repair is noise reduction against a profile learned from a silent selection, de-click by transient detection and cubic interpolation, and de-breath by detection of quiet broadband runs between phrases with a gain dip the person sets — each a step applied to the whole lane or one region, heard at once, undone at once, replayed from the take's own samples so the file on the shelf is never written. Splice cuts the lane at a boundary and joins another take at its own with an equal-power crossfade of a length the person sets; punch-in records over the region between an in and an out point with the crossfade at both ends. Keeping a repair, a splice or a punch writes a NEW take through `write_take_wav` (`src-tauri/src/studio.rs`), never over the one it came from. Proofs: `.journals/proofs/repair-a-noisy-take.mjs` · `splice-two-takes.mjs` · `punch-into-a-region.mjs` · `wav-bytes-round-trip.mjs`.

**Master and land.** The mix rendered to one buffer in the window and read to spec: integrated loudness per ITU-R BS.1770-4 (K-weighting, 400 ms blocks at 75% overlap, the −70 LUFS absolute gate and the −10 LU relative gate), momentary and short-term maxima, and true peak by the standard's 4× reconstruction — so a crest that falls *between* two samples is seen, where a sample-peak meter would miss it (`src/lib/master.ts`). **Master** gains to a target the person sets (−14, −16 for spoken word, or −23) and holds the result under a ceiling in dBTP (default −1) with a look-ahead limiter, reported before and after in both figures; deep limiting costs loudness and the report says where it landed rather than pretending. The master is a NEW buffer — no lane and no take on the shelf is written by any of it. **Land** writes a **chapterized WAV**: the mixdown plus a `cue ` chunk and a `LIST`/`adtl` chunk of `labl` entries, one per chapter mark, both *after* the data chunk so a reader that knows neither still opens the sound (`src/lib/container.ts`); it lands as a take like any other. Chapter marks live in the session document beside the lanes. **Encode-out is whatever the platform already carries** — `MediaRecorder.isTypeSupported` is asked, and where it says Opus in WebM or Ogg, an Opus file lands beside the WAV with its chapters as an ffmetadata `-chapters.txt` and a WebVTT `-chapters.vtt`, because WebM chapters need a muxer this app does not have. **MP3 and AAC are walls**: no encoder ships here and no browser recorder writes MP3 — the panel says so in those words and offers WAV and Opus. The no-ffmpeg promise holds: no ffmpeg, no native codec dependency, nothing leaving the device. Proofs: `.journals/proofs/loudness-to-spec.mjs` · `true-peak-over-samples.mjs` · `limit-to-a-ceiling.mjs` · `chapters-in-a-wav.mjs`.
...
│   ├── master.ts · container.ts   # loudness, true peak, the limiter · the chapterized WAV
│   ├── encode.ts                  # what the platform's own MediaRecorder offers
